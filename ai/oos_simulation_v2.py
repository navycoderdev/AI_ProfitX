"""Offline four-symbol OOS simulation with broker-verified contract economics."""
...
def simulate_locked_oos_v2(model, rows, *, threshold, contracts):
    if model.version != "Brain-v2" or set(contracts) != {"EURUSD", "GBPUSD", "USDJPY", "XAUUSD"}:
        raise ValueError("Brain-v2 and verified four-symbol contracts are required.")
    if not rows or any(row["split"] != "OOS" or row["symbol"] not in contracts for row in rows):
        raise ValueError("Only frozen four-symbol OOS rows can be simulated.")
    ...
    for row in sorted(rows, key=lambda item: (item["timestamp"], item["symbol"])):
        ...
        symbol_predictions[symbol][action.value] += 1
        if action is ProposalAction.NO_TRADE:
            continue
        if symbol in last_exit and row["timestamp"] <= last_exit[symbol]:
            skipped[symbol] += 1
            continue
        entry, exit_price, spread = row["entry_open"], row["exit_close"], row["entry_spread_points"]
        if entry is None or spread is None:
            # The broker supplied no candle at this decision time (session closure).
            unfilled[symbol] += 1
            continue
    ...
    result["per_symbol"] = {symbol: {**_summary([trade for trade in trades if trade["symbol"] == symbol],
        ...
        "coverage_status": "AVAILABLE" if symbol_predictions[symbol] else "NO_OOS_ROWS"}
    ...
    result["cost_assumptions"] = {"lots": {symbol: contract.lots for symbol, contract in contracts.items()},
        ...
        "pnl": "contract_size_times_lots;_JPY_converted_at_exit_close",
