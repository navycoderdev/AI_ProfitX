"""Train immutable Brain-v3 on Dataset v4; lock before opening OOS."""
...
def main() -> None:
    ...
    if dataset["state"] != "FROZEN" or dataset["dataset_version"] != DATASET_VERSION or dataset["leakage"] != "PASS":
        raise RuntimeError("Dataset v4 is not eligible for training.")
    rows, names, label_meta = BrainV1Dataset(sessions, dataset["dataset_hash"], DATASET_VERSION).rows()
    groups = {split: [row for row in rows if row["split"] == split] for split in ("TRAIN", "VALIDATION", "OOS")}
    if len(names) != 22 or any({row["symbol"] for row in group} != set(SYMBOLS) for group in groups.values()):
        raise RuntimeError("Brain-v3 feature/universe gate failed.")
    artifact = ModelArtifactManager()
    path = artifact.directory / f"{VERSION}.json"
    if path.exists():
        raise RuntimeError("Immutable Brain-v3 artifact already exists.")
    ...
    artifact.save(model, {"dataset_version": DATASET_VERSION, "dataset_hash": dataset["dataset_hash"],
        "feature_set_version": "feature-set-v1", "label_spec": LABEL_SPEC_V1,
        "confidence_policy": {"version": "confidence-policy-v3", "threshold": threshold},
        "validation": validation, "seed": 0, "market_data_source": dataset["source"]})
    digest = sha256(path.read_bytes()).hexdigest()
    # OOS is accessed only after the model and policy artifact is written and hashed.
    ...
    result = {"model_version": VERSION, "status": "CANDIDATE", "artifact_path": str(path),
        "artifact_hash": digest, "dataset_version": DATASET_VERSION, "dataset_hash": dataset["dataset_hash"],
        "market_data_source": dataset["source"],
        "confidence_policy": {"version": "confidence-policy-v3", "threshold": threshold},
        "rows": {split: len(group) for split, group in groups.items()}, "validation": validation, "oos": oos}
    registry = ModelRegistry(sessions, audit=AuditRepository(sessions))
    metadata = ModelMetadata(VERSION, "Brain-v2", {"dataset_version": DATASET_VERSION,
        "dataset_hash": dataset["dataset_hash"], "market_data_source": dataset["source"],
        "feature_names": list(names), "train_rows": len(groups["TRAIN"]),
        "validation_rows": len(groups["VALIDATION"]), "oos_rows": len(groups["OOS"])}, {},
        "feature-set-v1", "interpretable_multinomial_logistic",
    ...
    registry.update_training_metadata(VERSION, label_spec=LABEL_SPEC_V1,
        ...
        label_accounting={"class_distribution": label_meta["class_distribution"], "exclusions": label_meta["exclusions"]})
    registry.transition(VERSION, ModelStatus.CANDIDATE)
    Path("reports/brain_v3_evaluation.json").write_text(json.dumps(result, indent=2, sort_keys=True), encoding="utf-8")
